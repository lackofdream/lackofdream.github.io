...
核心思路是利用多个 -d 参数可以清空指定文件，同时配合另一个进程的写入来实现文件开头为\x00，根据 login 函数实现：
...
如果 PWFILE 这个文件有内容且第一个字节是 \x00，那么 fgets 拿到的就会是一个空字符串，这时我们直接输入 login （即调用 login("")） 就可以成功登录了。

先打开进程 A，debug 日志文件指定为 password 文件
