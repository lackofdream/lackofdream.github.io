...
其实做的时候还挺纳闷，要登陆的是 flag08 用户，这包里登录的是 level8。先不管这个，包里可以看到 client 输入了 `b'level8\x0dbackdoor\x7f\x7f\x7f00Rm8\x7fate\x0d'`，对照 ASCII 表可知 \x0d 就是 \r，\x7f 是 DEL 即退格。所以包里client输入的密码是 backd00Rmate，用这个密码登录 flag08 即可。
